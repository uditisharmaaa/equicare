...
export default function App() {
  ...
  function copyToClipboard() {
    navigator.clipboard.writeText(transcript.trim()).catch(console.error);
  }
      ...
      <div className="w-full max-w-3xl p-8 rounded-2xl border border-blue-100 bg-white shadow-lg">
        <header className="mb-6 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm text-blue-600" htmlFor="lang">Lang</label>
            <select
              id="lang"
        ...
        {!supported ? (
          <p className="text-sm text-red-500">
            Your browser doesn’t support SpeechRecognition. Try Chrome on desktop.
          </p>
        ) : (
          <>
            {/* Doctor selection */}
            <div className="mb-6">
              <label className="block mb-2 text-sm font-medium text-blue-600" htmlFor="doctor">
                Select Doctor
              </label>
              <select
                id="doctor"
            ...
            <div className="mt-6 grid gap-3">
              ...
              {interim && (
                <p className="text-xs text-gray-400">
                  Listening… (live words are being captured)
                </p>
              )}
            </div>
